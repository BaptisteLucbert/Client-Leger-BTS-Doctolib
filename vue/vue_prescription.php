...
<table>
    ...
    if (isset($lesPrescriptions)) {
        foreach ($lesPrescriptions as $unPrescription) {
            ...
            echo "<td>" . $unPrescription['priseMedicament'] . "</td>";

            $patientInfo = $unControleur->selectWherePatient($unPrescription['idpatient']);
            echo "<td>" . $patientInfo['nom'] . " " . $patientInfo['prenom'] ."</td>";

            $medecinInfo = $unControleur->selectWhereMedecin($unPrescription['idmedecin']);
